...
from contact_mcts.envs.collision import NamedCollisionObject, CollisionDetector

class FingerDoubleAndBox(BulletEnv):
    resources_path = pathlib.Path(__file__).parent.absolute()/'resources'

    def __init__(self, params, box_pos, box_orn, ee_init=None, server=pybullet.GUI, dt=0.001):
        ...
        # Zoom onto the robot
        pybullet.resetDebugVisualizerCamera(0.65, 30, -45, 
                                           (0, 0., 0.))
        ...
        pybullet.changeDynamics(
            self.finger0.robot_id, self.finger0.bullet_endeff_ids[0], 
            lateralFriction=mu_c)
        pybullet.changeDynamics(
            self.finger1.robot_id, self.finger1.bullet_endeff_ids[0], 
            lateralFriction=mu_c)

        # Reset the robot to some initial state.
        dq = np.zeros(self.finger0.nv)
        q = np.array([0., np.pi/3, -(np.pi/2 + np.pi/3)])
        self.q_default = q
        ...
        if ee_init is None:
            ...
            q0, _ = inverse_kinematics_3d(self.finger0.pin_robot, ee0_init,
                                          self.ee0_id, q_null=q)
            q1, _ = inverse_kinematics_3d(self.finger1.pin_robot, ee1_init,
                                          self.ee1_id, q_null=q)
            self.finger0.reset_state(q0, dq)
            self.finger1.reset_state(q1, dq)

        # add a plane
        plane_urdf = str(self.resources_path/'plane.urdf')
        self.plane_id = pybullet.loadURDF(plane_urdf)
        pybullet.resetBasePositionAndOrientation(
            self.plane_id, [0., 0., params.ground_height], (0., 0., 1., 1.))
        pybullet.changeDynamics(
            self.plane_id, -1, lateralFriction=mu_e)

        # add a box
        box_urdf = str(self.resources_path/'box.urdf')
        self.box_id = pybullet.loadURDF(box_urdf)
        pybullet.resetBasePositionAndOrientation(
            self.box_id, box_pos, box_orn)
        pybullet.changeDynamics(self.box_id, -1, lateralFriction=mu_c, 
                                rollingFriction=0.0, spinningFriction=0.02)
    ...
    def add_visual_frame(self, pos, orn, length=0.1, width=1):
        ...
        pybullet.addUserDebugLine(pos, end_point_x, lineColorRGB=[1, 0, 0], lineWidth=width)
        pybullet.addUserDebugLine(pos, end_point_y, lineColorRGB=[0, 1, 0], lineWidth=width)
        pybullet.addUserDebugLine(pos, end_point_z, lineColorRGB=[0, 0, 1], lineWidth=width)
    ...
    def stop_video_recording(self):
        ...
        if hasattr(self, "logging_id"):
            pybullet.stopStateLogging(
                pybullet.STATE_LOGGING_VIDEO_MP4, self.logging_id
            )

    def create_collision_detector(self):
        collision_bodies0 = {"robot": self.finger0.robot_id,
                             "box": self.box_id,}
        collision_bodies1 = {"robot": self.finger1.robot_id,
                             "box": self.box_id,}
        ...
        col_detector0 = CollisionDetector(
            ...
            self.finger0,
            collision_bodies0,
            [(finger0_upper_link, box), (finger0_middle_link, box), (finger0_lower_link, box)],
        )
        ...
        col_detector1 = CollisionDetector(
            ...
            self.finger1,
            collision_bodies1,
            [(finger1_upper_link, box), (finger1_middle_link, box), (finger1_lower_link, box)],
        )
