...
def contact_schedule(contacts, contact_mode, params):
    ...
    for c in range(params.n_contacts):
        for s in range(params.n_surfaces):
            ...
            r_constrained = constrained_location(contact.wr_b, params.simplices[s], params)
...
def environment_contact(location, orientation, params,
                        type='sticking', sliding_direction=None):
    ...
    if type == 'sticking':
        ...
        constr += sliding_friction(f_local, sliding_direction, params.environment_friction)
...
def torque_approximation(contact, params):
    ...
    lb_r = params.max_location * np.array([-1 + 2 * k / n_envelopes for k in range(n_envelopes)])
    ub_r = params.max_location * np.array([-1 + 2 * (k + 1) / n_envelopes for k in range(n_envelopes)])
    ...
    for m in range(6):
        ...
        for k in range(n_envelopes):
            for l in range(2):
                ...
                constr += (-b[m] + r[i] * lb_f[l] + lb_r[k] * f[j] - lb_r[k] * lb_f[l] <= M * (1 - z))
                constr += (-b[m] + r[i] * ub_f[l] + ub_r[k] * f[j] - ub_r[k] * ub_f[l] <= M * (1 - z))
                constr += (b[m] - r[i] * lb_f[l] - ub_r[k] * f[j] + ub_r[k] * lb_f[l] <= M * (1 - z))
                constr += (b[m] - r[i] * ub_f[l] - lb_r[k] * f[j] + lb_r[k] * ub_f[l] <= M * (1 - z))
    ...
    for m in range(6):
        ...
        for k in range(n_envelopes):
            for l in range(2):
                ...
                constr += (-b[m] + r[i] * lb_f[l] + lb_r[k] * f[j] - lb_r[k] * lb_f[l] <= M * (1 - z[m, env_id]))
                constr += (-b[m] + r[i] * ub_f[l] + ub_r[k] * f[j] - ub_r[k] * ub_f[l] <= M * (1 - z[m, env_id]))
                constr += (b[m] - r[i] * lb_f[l] - ub_r[k] * f[j] + ub_r[k] * lb_f[l] <= M * (1 - z[m, env_id]))
                constr += (b[m] - r[i] * ub_f[l] - lb_r[k] * f[j] + lb_r[k] * ub_f[l] <= M * (1 - z[m, env_id]))
