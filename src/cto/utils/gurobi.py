...
class MVar(object):
    ...
    def __repr__(self):
        return "Gurobi Matrix of {nrows} rows and {ncols} cols".format(nrows=self.nrows, 
                                                                       ncols=self.ncols)
    ...
    def __getitem__(self, index):
        if isinstance(index, (int, slice)):
            ...
            if isinstance(row, int) and isinstance(col, int):
                return self.var[row][col] # do not return a MVar object for a single element
    ...
    def __eq__(self, rhs):
        ...
        if hasattr(rhs, "__len__"):
            ...
            elif ((len(rhs.shape) == 1 or rhs.shape[0] == 1) and
                   self.ncols == 1 and self.nrows==sum(rhs.shape)):
                rhs = rhs.reshape((-1, 1))
                for m in range(self.nrows):
                    for n in range(self.ncols):
                        constr.append(self[m, n] == rhs[m, n])
