...
def random_poses_composite(params, n_desired_poses=2, primitive='sc'):
    ...
    ub = np.array([ 0.08,  0.08, z, 0., 0.,  np.pi])
    # init region
    init_lb = np.array([-0.05, -0.05, z, 0., 0., -np.pi/2])
    init_ub = np.array([ 0.05,  0.05, z, 0., 0.,  np.pi/2])

    if primitive == 'sc':
        diff_lb = np.array([-0.05, -0.05, 0, 0., 0., -np.pi / 4])
        diff_ub = np.array([ 0.05,  0.05, 0, 0., 0.,  np.pi / 4])

        desired_poses = generate_random_poses(n_desired_poses, lb, ub, 
                                              diff_lb, diff_ub,
                                              init_lb, init_ub)
        ...
        desired_poses = generate_random_poses(n_desired_poses - 1, lb, ub, 
                                              diff_lb, diff_ub,
                                              init_lb, init_ub)
...
def random_poses_primitive(params, primitive, init_pose=None):
    ...
    if primitive == 'r':
        ...
        desired_poses = [init_pose,
                         init_pose + np.array([dx, 0, dz, 0, rot, 0])]
        return desired_poses
    ...
    desired_poses = generate_random_poses(1, lb, ub, 
                                          diff_lb, diff_ub,
                                          init_lb, init_ub)
    return desired_poses
...
def exp_primitive(primitive, object_urdf, robot_config, trained_networks,
                  save_results=True, n_trials=50, mcts_iter=100):
...
def exp_composite(primitive, n_desired_poses, 
                  object_urdf, robot_config, trained_networks,
                  save_results=True, n_trials=50, mcts_iter=200, include_miqp=True):
    # store and compute the metrics
    errors = []
    errors_miqp = []
    errors_untrained = []
