...
from cto.mcts.contacts import skew_symetrify, fixed_force_contact, fixed_location_contact
...
class ForceProblem(object):
    ...
    def setup(self):
        ...
        # n: time index along the whole trajectory
            ...
            self.costs.append(1e4 * cp.sum_squares(total_force - self.params.traj_desired.total_force[n]))
            # euler equation 
            total_torque = cp.sum([c.torque for c in curr_contacts])
            self.costs.append(1e6 * cp.sum_squares(total_torque - self.params.traj_desired.total_torque[n]))
...
class LocationProblem(object):
    def __init__(self, forces, contact_plan, params):
        ...
        self.params = params
        self.forces = forces
        self.sticking = cp.Parameter((self.horizon, self.params.n_contacts), 
                                      value=np.zeros((self.horizon, self.params.n_contacts)))
        self.environment_torque = cp.Parameter((self.horizon, 3), 
                                                value=np.zeros((self.horizon, 3)))
    ...
    def set_environment_torque(self):
        for i in range(self.horizon):
            ...
            self.environment_torque.value[i] = np.sum(ec.torque.value for ec in self.params.environment_contacts[n]) 
        ...
        for i in range(self.horizon):
            ...
            for c, s in enumerate(curr_mode):
                ...
                if i >= 1:
                    ...
                    self.constr.append(self.sticking[i, c] * (ffc.location_weights - prev_ffc.location_weights) == 0)
            
            # environment contact locations are fixed, no need to add into contact list
            self.contacts.append(curr_contacts)

            # euler equation
            total_torque = cp.sum([ffc.torque for ffc in curr_contacts]) + self.environment_torque[i]
            self.costs.append(1e6 * cp.sum_squares(total_torque - self.params.traj_desired.total_torque[n]))
...
def integrate_solution(sol, params, idx_start=None, idx_end=None):
    ...
    traj = integrate_trajectory(pose_start, idx_start, idx_end, 
                                total_force, total_torque, params)
    return traj
